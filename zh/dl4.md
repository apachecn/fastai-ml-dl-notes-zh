...
*   `p=0.99`将抛弃99％的激活。 不会过度适应并且非常适合概括，但会破坏你的准确率。
...
你可以通过设置`ps=0.`来删除dropout `ps=0.` 但即使在几个迭代之后，我们开始大规模过拟合（训练损失«验证损失）：
...
**问题** ：为什么要监控损失而不是准确率？  [[23:53](https://youtu.be/gbceqO8PpBg%3Ft%3D23m53s)] 损失是我们唯一可以看到的验证集和训练集。 正如我们后来所了解的那样，损失是我们实际上正在优化的事情，因此更容易监控和理解这意味着什么。
...
*   按照惯例，我们将从创建模型数据对象开始，该对象具有内置于其中的验证集，训练集和可选测试集。 从那以后，我们将获得一个学习器，然后我们可以选择调用`lr_find` ，然后调用`learn.fit`等等。
...
*   `metrics` ：这是一个自定义指标，它指定在每个迭代结束时调用的函数并打印出结果
...
对于这些类型的模型，事实上对于许多模型，我们构建的模型取决于数据。 在这种情况下，我们需要知道我们有什么嵌入矩阵。 所以在这种情况下，数据对象创建了学习器（颠倒到我们之前看到的）：
...
*   一个巧妙的技巧torchtext做的是每次随机改变`bptt`数，所以每个迭代它的文本略有不同 - 类似于计算机视觉中的洗牌图像。 我们不能随意改变这些词，因为它们需要按正确的顺序排列，所以我们会随机移动它们的断点。
...
语言建模的准确率通常使用度量_复杂_度来度量，这只是我们使用的损失函数的`exp()` 。
...
现在你可以继续调用`get_model`来获取我们的学习器。 然后我们可以加载预训练的语言模型（ `load_encoder` ）。
